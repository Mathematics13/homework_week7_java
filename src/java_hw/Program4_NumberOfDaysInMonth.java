...
/** Program4_NumberOfDaysInMonth
 * This program has two methods 1) isLeapYear with int as parameter and find whether the given year is a leap year
 * 2) getDaysInAMonth - gives the output of no of days in a month for a given month and year
 *
 */
public class Program4_NumberOfDaysInMonth
{
    public static void main(String[] args)
    {
        ...
        int year = 2000;                                                    //Can change the year here to test
        ...
        System.out.println("get days in a month.. "+getDaysInMonth(month,year));

    }
    public static boolean isLeapYear(int year) {
        if (year < 1 || year > 9999) {
            System.out.println("Parameter is out of the range ");
            return false;
        } else {
            switch (year % 4) {
                case 0:
                    if (year % 100 == 0)                                  //if divisible by 100, its only a leap year if also divisible by 400
                    {
                        return year % 400 == 0;
                    } else {
                        return true;
                ...
                default:
                    System.out.println("Return false  since Not a leap year  ");
                    return false;
            }
        }
    }
